...
Remix v1 was released in 2021. [A lot has changed since then](https://blog.woolgarmedia.co.uk/blog/javascript). The web platform is more powerful than ever, and there's an entire new generation of developers learning the web `Request` and `Response` APIs first instead. They're able to learn once, and write the same JavaScript everywhere, reusing their existing knowledge. I love that.
...
export async function GET(request: Request) {
  const res = await fetch('https://api.blog.woolgarmedia.co.uk/...', { ... } )
...
From my post earlier this year on [funding open source](https://blog.woolgarmedia.co.uk/blog/funding-open-source):
